  ...
  const subscription = web3.eth.subscribe('pendingTransactions', function (error,result){
    ...
    .on('data', async (txHash) => {
      try {
        ...
        const trx = await getTxWithRepeat(3, txHash)

        if (trx && trx.from.toLowerCase() === process.env.TARGET_WALLET.toLowerCase()){
            ...
            if (decodedData){
                ...
                if (process.env.ONLY_SUCCESSFUL === 'true'){
                    ...
                    confirmAndSendEtherTransaction(txHash, 1, decodedData, trx, sendTx)
                }else {
...
function confirmAndSendEtherTransaction(txHash, confirmations = 1, decodedData, trx, callback) {
    setTimeout(async () => {
        ...
        switch (decodedData.name){
            case 'swapExactETHForTokens':
                ...
                if (amount.isGreaterThan(MAX_AMOUNT)){
                    const result = await SDK.createTransactionExactTokenToToken(process.env.WALLET_FROM, process.env.WALLET_FROM_PRIVATE_KEY, process.env.WALLET_FROM, MAX_AMOUNT.toNumber(), process.env.ROUTER_ADDRESS, tokenIn, tokenOut)
                    if (result){
                        console.log('Bot Buys: ', result)
                    }
                }else if (amount.isGreaterThan(MIN_AMOUNT) && amount.isLessThan(MAX_AMOUNT)){
                    const result = await SDK.createTransactionExactTokenToToken(process.env.WALLET_FROM, process.env.WALLET_FROM_PRIVATE_KEY, process.env.WALLET_FROM, amount.toNumber(), process.env.ROUTER_ADDRESS, tokenIn, tokenOut)
                    if (result){
                        console.log('Bot Buys: ', result)
                    }
                }
            ...
            case 'swapExactTokensForETH':
                const amountOutMin = decodedData.params.filter(el => el.name === 'amountOutMin')[0]
                path = decodedData.params.filter(el => el.name === 'path')[0].value
                tokenOut = WETH
                tokenIn = path[0]
                amount = SDK.getNormalizedNumber(amountOutMin.value, 18)
                if (amount.isGreaterThan(MAX_AMOUNT)){
                    const result = await SDK.createTransactionTokensForExactETH(process.env.WALLET_FROM, process.env.WALLET_FROM_PRIVATE_KEY, process.env.WALLET_FROM, MAX_AMOUNT.toNumber(), process.env.ROUTER_ADDRESS, tokenIn, tokenOut)
                    if (result){
                        console.log('Bot Sells: ', result)
                    }
                }else if (amount.isGreaterThan(MIN_AMOUNT) && amount.isLessThan(MAX_AMOUNT)){
                    const result = await SDK.createTransactionTokensForExactETH(process.env.WALLET_FROM, process.env.WALLET_FROM_PRIVATE_KEY, process.env.WALLET_FROM, amount.toNumber(), process.env.ROUTER_ADDRESS, tokenIn, tokenOut)
                    if (result){
                        console.log('Bot Sells: ', result)
                    }
                }
    ...
    if (logs && logs.length > 0) {
        ...
        if (swapLogs.length > 0){
            for (let swapLog of swapLogs){
                ...
                if (amount0In > 0){
                    if (amount1Out > 0){
                        const amount0 = getNormalizedNumber(amount0In, token0decimals).toFixed(4)
                        const amount1 = getNormalizedNumber(amount1Out, token1decimals).toFixed(4)
                        console.log('New Swap! ', amount0, ' ', token0Sym, ' -> ', amount1, ' ', token1Sym)
                        if (token0.toLowerCase() === WETH || token1.toLowerCase() === WETH){
                            console.log('Executing Transation')
                            await createTransaction(token0Sym, token1Sym, amount0, amount1, token0, token1)
                        }
                ...
                if (amount1In > 0){
                    if (amount0Out > 0){
                        const amount1 = getNormalizedNumber(amount1In, token1decimals).toFixed(4)
                        const amount0 = getNormalizedNumber(amount0Out, token0decimals).toFixed(4)
                        console.log('New Swap! ', amount1, ' ', token1Sym, ' -> ', amount0, ' ', token0Sym)
                        if (token0.toLowerCase() === WETH || token1.toLowerCase() === WETH){
                            console.log('Executing Transation')
                            await createTransaction(token0Sym, token1Sym, amount0, amount1, token0, token1)
                        }
...
async function createTransaction(token0Sym, token1Sym, amount0, amount1, token0Address, token1Address){
    try{
        if (token0Address.toLowerCase() === WETH){
            switch (amount0){
                case amount0 > MAX_AMOUNT:
                    SDK.createTransactionExactTokenToToken(process.env.MAIN_WALLET, process.env.PRIVATE_KEY, process.env.MAIN_WALLET, amount0, process.env.ROUTER_ADDRESS, token0Address, token1Address)
                        .then(hash => console.log('Transaction success: ', hash))
                        .catch(error => console.log(error))
                    break
                case amount0 > MIN_AMOUNT && amount0 < MAX_AMOUNT:
                    SDK.createTransactionExactTokenToToken(process.env.MAIN_WALLET, process.env.PRIVATE_KEY, process.env.MAIN_WALLET, amount0, process.env.ROUTER_ADDRESS, token0Address, token1Address)
                        .then(hash => console.log('BOT TX EXECUTED: ', token0Sym , ' -> ', token1Sym))
                        .catch(error => console.log(error))
                    break
            }
        }
        if (token1Address.toLowerCase() === WETH){
            switch (amount1){
                case amount1 > MAX_AMOUNT:
                    // sell all tokens
                    SDK.createTransactionExactTokenToToken(process.env.WALLET_FROM, process.env.WALLET_FROM_PRIVATE_KEY, process.env.WALLET_FROM, MAX_AMOUNT, process.env.ROUTER_ADDRESS, token1Address, token0Address)
                        .then(hash => console.log('Transaction success: ', hash))
                        .catch(error => console.log(error))
                    break
                case amount1 > MIN_AMOUNT && amount1 < MAX_AMOUNT:
                    SDK.createTransactionExactTokenToToken(process.env.WALLET_FROM, process.env.WALLET_FROM_PRIVATE_KEY, process.env.WALLET_FROM, amount1, process.env.ROUTER_ADDRESS, token1Address, token0Address )
                        .then(hash => console.log('Transaction success: ', hash))
                        .catch(error => console.log(error))
                    break
            }
